...
class GridUtils {
  ...
  calculateNextGeneration = (grid, day) => {
    ...
    for (let i = 0; i < gridSize; i++) {
      ...
      for (let j = 0; j < gridSize; j++) {
        ...
        if (airPollution) {
          newCell.newAirPollution =
            configuration.temperature.pollutionTemperature.value +
            airPollution / 1000;
        }
  ...
  computeIceLevel = (newTemp, oldCell) => {
    return newTemp - oldCell.initialTemperature > 2
      ? oldCell.iceLevel - 1
      : oldCell.iceLevel;
  };
  ...
  computeNewTemperature = (
    ...
    let tempReducers = isForrest
      ? configuration.temperature.forrestTemperature.value
      : 0;
    tempReducers += isRaining
      ? configuration.temperature.rainTemperature.value
      : 0;

    let tempRaisers = airPollution
      ? configuration.temperature.pollutionTemperature.value +
        airPollution / 1000
      : 0;
