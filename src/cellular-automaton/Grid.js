...
class Grid extends React.Component {
  ...
  start = () => {
    this.timerID = setInterval(
      () =>
        this.setState({
          grid: gridUtils.calculateNextGeneration(
            this.state.grid,
            this.state.days + 1
          ),
  ...
  restartValues = () => {
    ...
    Object.keys(configuration.temperature).forEach(
      key => (configuration.temperature[key].value = 0.01)
    );
  };
        ...
        <div className="info-section">
          <div className="cell-info">
            ...
            {!this.state.selectedCell && (
              <div>
                No Cell Selected. Click on a cell to view it's properties.
              </div>
            )}
            {this.state.selectedCell &&
              Object.keys(this.state.selectedCell).map(key => {
                return (
                  <div key={key}>
                    <span>
                      <strong>{`${key}: `}</strong>
            ...
            {Object.keys(this.state.temperatureValues).map(key => {
              return (
                <div key={key} style={{ marginTop: "10px" }}>
                  ...
                  >
                    <strong>{`${
                      this.state.temperatureValues[key].displayName
                    }: `}</strong>
                  </div>{" "}
                  <input
                    type="number"
                    ...
                    value={this.state.temperatureValues[key].value}
                  />
                </div>
              );
            })}
        ...
        {this.state.showGraph && this.state.days >= 365 && (
          <div className="charts">
            ...
            <LineChart
              width={1400}
              height={800}
              data={configuration.graphValues}
            >
              <XAxis dataKey="day" />
              <YAxis />
